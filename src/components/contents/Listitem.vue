    ...
    <ul class="fly-list">
      <li v-for="(item,index) in items" :key="'list' + index">
        ...
        <div class="fly-list-badge" v-show="item.tags.length > 0">
          <span class="layui-badge layui-bg-red" v-for="(tag,index) in item.tags" :key="'tag' + index" :class="tag.calss">{{tag.name}}</span>
        </div>
      </li>
    </ul>
    <div style="text-align: center" v-show="isShow">
      <div class="laypage-main">
        <a @click.prevent="more()" href="jie/index.html" class="laypage-next">更多求解</a>
      </div>
    </div>
  </div>
</template>
